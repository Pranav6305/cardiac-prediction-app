...
const HeartWiseDashboard = () => {
  const [userData] = useState({
    ...
    dailyStats: {
      ...
      sleep: { current: 7.2, goal: 8, label: 'Last night\'s sleep', unit: 'hours' },
      water: { current: 2.5, goal: 3, label: 'Today\'s hydration', unit: 'liters' },
      calories: { current: 650, goal: 800, label: 'Estimated daily burn', unit: 'kcal' }
    },
    trends: {
    ...
    heartRisk: 18,
    notifications: [
      { type: 'meal', message: 'Time to log your lunch!', time: '12:30 PM', action: 'Log Meal' },
      { type: 'activity', message: 'Go for a 30-minute walk today.', time: '2:00 PM', action: 'View Exercises' },
      { type: 'sleep', message: 'Sleep Goal Update', time: '8:00 AM', action: null }
    ]
  });
    ...
    return (
      <div className="sleep-chart">
        <svg viewBox="0 0 300 100" className="sleep-line-chart">
          <polyline
            points={data.map((sleep, index) => `${(index * 45) + 20},${80 - (sleep * 8)}`).join(' ')}
            fill="none"
            stroke="#4F46E5"
            strokeWidth="2"
          />
          ...
          <div className="summary-grid">
            <div className="summary-card">
              ...
              <div className="card-content">
                ...
                <div className="metric">
                  <span className="value">{userData.dailyStats.steps.current}</span>
                  <span className="unit">steps</span>
                </div>
                <p className="label">{userData.dailyStats.steps.label}</p>
                <p className="goal">Goal: {userData.dailyStats.steps.goal} steps</p>
                <div className="progress-bar">
                  <div 
                    className="progress-fill steps"
                    style={{ width: `${getProgressPercentage(userData.dailyStats.steps.current, userData.dailyStats.steps.goal)}%` }}
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div className="summary-card">
              ...
              <div className="card-content">
                ...
                <div className="metric">
                  <span className="value">{userData.dailyStats.sleep.current}</span>
                  <span className="unit">hours</span>
                </div>
                <p className="label">{userData.dailyStats.sleep.label}</p>
                <p className="goal">Goal: {userData.dailyStats.sleep.goal} hours</p>
                <div className="progress-bar">
                  <div 
                    className="progress-fill sleep"
                    style={{ width: `${getProgressPercentage(userData.dailyStats.sleep.current, userData.dailyStats.sleep.goal)}%` }}
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div className="summary-card">
              ...
              <div className="card-content">
                <h4>Water Intake</h4>
                <div className="metric">
                  <span className="value">{userData.dailyStats.water.current}</span>
                  <span className="unit">liters</span>
                </div>
                <p className="label">{userData.dailyStats.water.label}</p>
                <p className="goal">Goal: {userData.dailyStats.water.goal} liters</p>
                <div className="progress-bar">
                  <div 
                    className="progress-fill water"
                    style={{ width: `${getProgressPercentage(userData.dailyStats.water.current, userData.dailyStats.water.goal)}%` }}
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div className="summary-card">
              ...
              <div className="card-content">
                ...
                <div className="metric">
                  <span className="value">{userData.dailyStats.calories.current}</span>
                  <span className="unit">kcal</span>
                </div>
                <p className="label">{userData.dailyStats.calories.label}</p>
                <p className="goal">Goal: {userData.dailyStats.calories.goal} kcal</p>
                <div className="progress-bar">
                  <div 
                    className="progress-fill calories"
                    style={{ width: `${getProgressPercentage(userData.dailyStats.calories.current, userData.dailyStats.calories.goal)}%` }}
                  ></div>
                </div>
              </div>
            </div>
        ...
        <section className="health-trends">
          ...
          <div className="trends-grid">
            <div className="trend-card">
              ...
              <p className="trend-description">Daily steps over the last week.</p>
            </div>
            <div className="trend-card">
              <h4>Sleep Trend</h4>
        ...
        <aside className="right-panel">
                ...
                <div className="notification-content">
                  <div className="notification-header">
                    <span className="notification-type">
                      {notification.type === 'meal' ? 'Meal Reminder' : 
                       notification.type === 'activity' ? 'Activity Suggestion' : 'Sleep Goal Update'}
                    </span>
          ...
          <section className="health-insights">
            ...
            <div className="risk-card">
              <h4>Heart Disease Risk: <span className="risk-percentage">{userData.heartRisk}%</span></h4>
              <p className="risk-description">
                Your current risk is moderate. Continue focusing on regular exercise and balanced meals. 
          ...
          <section className="health-resources">
            ...
            <div className="resources-grid">
              <div className="resource-card">
                ...
                <p>Learn how going green can significantly benefit your heart health.</p>
              </div>
              <div className="resource-card">
                <div className="resource-image exercise"></div>
                <h5>Simple Exercises for a Stronger Heart</h5>
                <p>Easy routines you can do at home to improve cardiovascular fitness.</p>
              </div>
              <div className="resource-card">
                <div className="resource-image lab"></div>
                <h5>Understanding Your Cholesterol Levels</h5>
                <p>A guide to interpreting your lipid profile and what the numbers mean.</p>
              </div>
              <div className="resource-card">
                <div className="resource-image meditation"></div>
                <h5>Managing Stress for Heart Health</h5>
                <p>Effective techniques to reduce stress and protect your cardiovascular system.</p>
              </div>
            </div>
          </section>
        </aside>
